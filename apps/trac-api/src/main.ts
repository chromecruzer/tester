...
// cloned import product
import GetAndUpdateProducts2 from "./app/GetAndUpdateProducts2";
//
...
const DEFAULT_PORT = 1234;    // change if original TRAC comes in action
...
// for downloading here is the router make sure for reference and it is used as atomic funtions  
  ...
  // clone by me  products
  const getProductData2 = new GetAndUpdateProducts2(configurations.postgresConfig, dataTableNames.products);
  app.get(`/${prefix}/products2`, getProductData2.getProducts(postgresClient));
  app.get(`/${prefix}/products2/uuid/:uuid`, getProductData2.getProducts(postgresClient));
  app.post(`/${prefix}/products2`, getProductData2.update(postgresClient));
