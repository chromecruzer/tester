...
export const AddItemsNoteDialog = ({batchEdit, open, requestClose, auditUuid}) => {
  ...
  const postAuditNotesMutation = useMutation({
    mutationFn: async () => {
      ...
      setError(null);
      handleClose();
      return postAuditNotes(batchEdit.createItemNote(note, 'Item'));
    },
  ...
  return <DialogModal open={open} requestClose={handleClose}>
    <div className='flex-col min-h-full min-w-full justify-items-stretch'>
      <h2 className='px-2 py-2 text-center font-bold text-bl-text-dark'>Add A Note To The Selected Items</h2>
      <textarea
        value={note}
        rows={12}
